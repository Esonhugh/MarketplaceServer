...
<section class="auth-page"><p class="eyebrow">MarketplaceServer</p><h1>Create account</h1>
...
{:else}<form class="panel form-stack" onsubmit={submit}><label for="register-username">Username</label><input class="field" id="register-username" autocomplete="username" required bind:value={username} /><label for="display-name">Display name</label><input class="field" id="display-name" required bind:value={displayName} /><label for="email">Email <span class="muted">(optional)</span></label><input class="field" id="email" type="email" autocomplete="email" bind:value={email} /><label for="new-password">Password</label><input class="field" id="new-password" type="password" autocomplete="new-password" required bind:value={password} />{#if error}<p class="error" role="alert">{error.message}{error.status === 409 ? ' Choose different account details.' : ''}</p>{/if}<button class="primary" disabled={pending} aria-busy={pending}>{pending ? 'Creating…' : 'Create account'}</button><a data-route href="/login">Already have an account?</a></form>{/if}</section>
