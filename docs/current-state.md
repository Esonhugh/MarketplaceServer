...
| Identity | 已实现 | 用户、个人 namespace、系统组、成员关系、管理员 bootstrap、Basic 认证 | `mod/backend/domain/identity/` |
| Personal Access Token | 已实现 | 创建、cursor 列表、撤销、scope 与可选过期时间；明文仅创建响应返回 | `mod/backend/domain/identity/token_service.go`, `mod/backend/handler/identity/tokens.go` |
...
| 完整管理前端 | 规划中 | 静态嵌入和 fallback 已实现，完整管理页面尚未交付 | `mod/frontend/`, `roadmap.md` |
...
`teams`、`plugins`、`versions`、`marketplaces` 和 `audit` 可以成为未来的 backend 内部领域，但不是当前已经存在的独立 package，也不是新的 kernel module。
...
|---|---|---|
| `GET` | `/api/v1/health` | 无 |
| `GET` | `/api/v1/me/tokens` | BasicAuth |
| `POST` | `/api/v1/me/tokens` | BasicAuth |
| `DELETE` | `/api/v1/me/tokens/:tokenId` | BasicAuth |
...
- personal access tokens 与 scopes
...
字段、索引和数据库 guard 以 `mod/backend/domain/*/model.go` 与 `migrate.go` 为准。当前没有 team、invitation、audit、outbox、job、session 或 SSH key migration。
...
密钥不应写入示例配置。Identity bootstrap 和 API-key pepper 等敏感值通过受保护的 runtime environment 提供。
...
- frontend 目前主要交付静态嵌入与 SPA 边界，不等于完整管理 UI。
